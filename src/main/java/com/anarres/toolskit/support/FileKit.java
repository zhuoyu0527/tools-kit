...
public class FileKit {
	...
	public static File[] ls(String path) {
		...
		throw new ToolBoxException(CharsetKit.format("Path [{}] is not directory!", path));
	}
	...
	public static List<String> listFileNames(String path) {
		...
		try {
			if (index == -1) {
				...
				for (JarEntry entry : Collections.list(new JarFile(jarPath).entries())) {
					...
					if (name.startsWith(subPath)) {
						String nameSuffix = CharsetKit.removePrefix(name, subPath);
			...
			throw new ToolBoxException(CharsetKit.format("Can not read file path of [{}]", path), e);
		}
		return paths;
	}
	...
	public static File copy(File src, File dest, boolean isOverride) throws IOException {
		...
		if (src.isDirectory()) {
			if (dest.isFile()) {
				throw new IOException(CharsetKit.format("Src [{}] is a directory but Dest [{}] is a file!", src.getPath(), dest.getPath()));
			}
	...
	public static void move(File src, File dest, boolean isOverride) throws IOException {
		...
		if (src.isDirectory() && dest.isFile()) {
			throw new IOException(CharsetKit.format("Can not move directory [{}] to file [{}]", src, dest));
		}
		...
		if (src.renameTo(dest) == false) {
			...
			try {
				...
				throw new IOException(CharsetKit.format("Move [{}] to [{}] failed!", src, dest), e);
			}
	...
	public static String getAbsolutePath(String path, Class<?> baseClass) {
		if (path == null) {
			path = CharsetKit.EMPTY;
		}
		if (baseClass == null) {
			return getAbsolutePath(path);
		}
		// return baseClass.getResource(path).getPath();
		return CharsetKit.removePrefix(PATH_FILE_PRE, baseClass.getResource(path).getPath());
	}
	...
	public static String subPath(String rootDir, File file) {
		...
		if (CharsetKit.isNotEmpty(rootDir) && CharsetKit.isNotEmpty(subPath)) {
	...
	public static String mainName(String fileName) {
		if (CharsetKit.isBlank(fileName) || false == fileName.contains(CharsetKit.DOT)) {
			return fileName;
		}
		return CharsetKit.subPre(fileName, fileName.lastIndexOf(CharsetKit.DOT));
	}
	...
	public static String extName(String fileName) {
		...
		int index = fileName.lastIndexOf(CharsetKit.DOT);
		if (index == -1) {
			return CharsetKit.EMPTY;
		} else {
			String ext = fileName.substring(index + 1);
			// 扩展名中不能包含路径相关的符号
			return (ext.contains(String.valueOf(UNIX_SEPARATOR)) || ext.contains(String.valueOf(WINDOWS_SEPARATOR))) ? CharsetKit.EMPTY : ext;
		}
	}
	// -------------------------------------------------------------------------------------------- name end
