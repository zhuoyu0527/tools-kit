...
public class RegexKit {
	...
	public static String extractMultiAndDelPre(Pattern pattern, String[] contents, String template) {
		...
		if (matcher.find()) {
			...
			contents[0] = CharsetKit.sub(content, matcher.end(), content.length());
			return template;
		}
		return null;
	}
	...
	public static String delPre(String regex, String content) {
		...
		if (matcher.find()) {
			return CharsetKit.sub(content, matcher.end(), content.length());
		}
		return content;
	}
